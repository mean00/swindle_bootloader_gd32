...
struct LN_FMCx
{
    ...
    uint32_t OPTKEY; // 08 OPTKEYR
    uint32_t STAT;   // 0C SR
    uint32_t CTL;    // 10 CR
    uint32_t ADDR;   // 14 AR
    uint32_t OBSTAT; // 18 OBR
    uint32_t WP;     // 1C WRPR
};
typedef volatile LN_FMCx LN_FMC;

#define LN_FMC_ADR (0x40022000)
#define fmc ((LN_FMC *)LN_FMC_ADR)

#define FLASH_KEYR (fmc->KEY)
#define FLASH_OPTKEYR (fmc->OPTKEY)
#define FLASH_SR (fmc->STAT)
#define FLASH_CR (fmc->CTL)
#define FLASH_AR (fmc->ADDR)
...
static void _flash_program_buffer_gd32(uint32_t address, uint16_t *data, unsigned len)
{
    if ((len & 3) || (address & 3))
    {
        _flash_program_buffer(address, data, len);
        return;
    }
